...
| `BASE_PATH` | `/Holad/` | Базовый путь веб-клиента. Укажите `/` для размещения в корне домена |
...
По умолчанию Holad настроен на базовый путь `/Holad/`. Это сделано для того, чтобы плеер можно было легко разместить за существующим Nginx-прокси вместе с другими сервисами (например, `https://example.com/Holad/`).

### Размещение в корне отдельного домена или поддомена (`/`)
Если вы выделяете под Holad отдельный домен или субдомен (например, `https://music.example.com/`), задайте переменную `BASE_PATH=/`:
...
При старте контейнера скрипт `entrypoint.sh` динамически адаптирует статические пути веб-клиента.
