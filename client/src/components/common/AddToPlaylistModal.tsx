...
export default function AddToPlaylistModal({ isOpen, onClose, trackIds }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div 
        className="bg-[#1c1c1c] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[80vh] overflow-hidden"
        onClick={(e) => e.stopPropagation()}
        onContextMenu={(e) => e.preventDefault()}
      >
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <h2 className="text-lg font-bold text-white">{t('common.add_to_playlist')}</h2>
          <button onClick={onClose} className="p-2 text-secondary hover:text-white transition-colors rounded-full hover:bg-white/10">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-white/10">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-secondary focus:outline-none focus:border-primary/50 transition-colors"
            />
          </div>
        </div>
            ...
            <div className="space-y-1">
              {filtered.map(p => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors text-left disabled:opacity-50 group"
                >
                  {p.coverArt ? (
                    <img src={getCoverArtUrl(p.coverArt, 100)} alt="" className="w-12 h-12 rounded object-cover shadow-sm" />
                  ) : (
                    <div className="w-12 h-12 rounded bg-white/5 flex items-center justify-center text-secondary">
                      <ListMusic size={20} />
                    </div>
                  )}
