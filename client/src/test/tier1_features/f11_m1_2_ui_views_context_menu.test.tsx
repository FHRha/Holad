...
describe('Milestone 1 UI Adversarial Suite: Context Menu & Downloads Integration', () => {
  ...
  describe('10. ContextMenu Action Invocations', () => {
    it.skip('triggers handleDownload when clicking Download option in ContextMenu', async () => {
      ...
      const downloadBtn = screen.getByText(/common\.download|Скачать/i);
      ...
      expect(handleDownloadSpy).toHaveBeenCalledWith('fresh-track-1', 'Fresh Track', 'track');
    ...
    it.skip('triggers remove_from_queue and mutates player queue when queueIndex is present', async () => {
      ...
      const removeQueueBtn = screen.getByText(/common\.remove_from_queue|Удалить из очереди/i);
      expect(removeQueueBtn).toBeTruthy();

      await act(async () => {
        fireEvent.click(removeQueueBtn);
      });

      expect(usePlayerStore.getState().queue.length).toBe(1);
      expect(usePlayerStore.getState().queue[0].id).toBe('queue-track-1');
      expect(useContextMenuStore.getState().isOpen).toBe(false);
    });
  });
});
